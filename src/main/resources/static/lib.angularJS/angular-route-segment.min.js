...
!function (a) {
    ...
    b.provider("$routeSegment", ["$routeProvider", function (b) {
        function c(a) {
        ...
        var e = this, f = e.options = {
            autoLoadTemplates: !0,
            strictMode: !1
        }, g = this.segments = {}, h = d(g, null), i = {};
        e.when = function (a, c) {
            return b.when(a, {segment: c}), i[c] = a, this
        }, a.extend(e, h), this.$get = ["$rootScope", "$q", "$http", "$templateCache", "$route", "$routeParams", "$injector", function (b, d, e, h, j, k, l) {
            function m(b) {
                var c = !1;
            ...
            function o(c, g, i) {
                ...
                return a.forEach(j, function (b, c) {
                ...
                }), i.template && (j.$template = i.template, a.isFunction(j.$template) && (j.$template = l.invoke(j.$template))), f.autoLoadTemplates && i.templateUrl && (j.$template = i.templateUrl, a.isFunction(j.$template) && (j.$template = l.invoke(j.$template)), j.$template = e.get(j.$template, {cache: h}).then(function (a) {
                    return a.data
                    ...
                    if (r.chain[c] = {
                        ...
                        var f = function () {
                            ...
                            return l.invoke(i.watcher, {}, {segment: r.chain[c]})
                        }, h = f();
            ...
            var r = {
                name: "", $routeParams: a.copy(k), chain: [], startsWith: function (a) {
                    var b = new RegExp("^" + a);
                    return b.test(r.name)
                }, contains: function (a) {
                    for (var b = 0; b < this.chain.length; b++)if (this.chain[b] && this.chain[b].name == a)return !0;
                    return !1
            ...
            return b.$on("$routeChangeSuccess", function (a, b) {
                ...
                if (c && c.segment) {
                    for (var e = c.segment, f = e.split("."), g = [], h = -1, i = 0; i < f.length; i++) {
                        var j = q(i, f);
                    ...
                    var k = d.when();
                    if (g.length > 0)for (var i = 0; i < g.length; i++)!function (a) {
                        k = k.then(function () {
                            return n(g[a].index, g[a].newSegment)
                        }).then(function (b) {
                            if (void 0 != b.success) {
                                p(b.success);
                                for (var c = g[a].index + 1; c < r.chain.length; c++)r.chain[c] && (r.chain[c] = null, n(c, null))
                            }
                        })
                    }(i);
                    k.then(function () {
                        if (r.chain.length > f.length) {
                            ...
                            r.chain.splice(-b, b);
                            for (var c = f.length; a > c; c++)n(c, null), h = r.chain.length - 1
                        }
                    }).then(function () {
                        var a = d.when();
                        if (h == r.chain.length - 1)for (var b = q(h, r.name.split(".")); b;) {
                            var c = b.children, e = h + 1;
                            b = null;
                            for (var f in c)!function (c, d, e) {
                                d[c].params["default"] && (a = a.then(function () {
                                    return n(e, {name: d[c].name, params: d[c].params}).then(function (a) {
    ...
    a.module("view-segment", ["route-segment"]).directive("appViewSegment", ["$route", "$compile", "$controller", "$routeParams", "$routeSegment", "$q", "$injector", "$timeout", "$animate", function (a, b, c, d, e, f, g, h, i) {
        return {
            restrict: "ECA", priority: 400, transclude: "element", compile: function (a, b) {
        ...
        return {
            restrict: "ECA", priority: -400, link: function (b, e) {
                var f = e.data("viewSegment") || {}, g = a.extend({}, f.locals), h = g && g.$template;
                h && e.html(h);
                var i = c(e.contents());
